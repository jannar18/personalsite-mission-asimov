...
├── AGENTS.md                # Agent context (Codex / other agents)
...
│   ├── moodboard/           # Visual references organized by source
│   └── asset-workflows/
├── notes/                   # Code review artifacts and working notes
└── .lattice/                # Task tracking (Lattice) — managed by Claude, see below
```
...
- **Soul / vibe** comes from Julianna's work (portfolio, mae graphics, pinterest board) — warm textures, stipple, watercolor, handcraft, materiality, architectural drawing language. This is what makes the site *hers*, not a generic Asimov clone.
- **Structure / execution** comes from Asimov Collective sites — editorial pacing, typographic hierarchy, bento grids, full-bleed sections, strong branding, restrained navigation, scroll flow. This is the level of web craft we're targeting.
...
Moodboard images live in `research/moodboard/` — organized by source:

**Asimov client sites (structure/execution reference):**
- `asimov-portfolio/` — Asimov Collective's own portfolio site
- `monumental-labs/` — monumentallabs.co (architecture/craft, warm cream palette)
- `venice-ai/` — venice.ai (warm tan/gold palette)
- `extropic/` — extropic.ai (vibrant yellow, dark theme)
- `rainmaker/` — rainmaker.com (deep forest green, editorial splits)
- `valar-atomics/` — valaratomics.com (dark/industrial, nuclear energy)
- `shinkei-systems/` — shinkeisystems.com (orange, robotics/fishing)

**Julianna's visual DNA (soul/vibe reference):**
- `mae graphics/` — texture/mark-making studies (stipple, charcoal, watercolor wash, geometric)
- `portfolio/` — architecture portfolio from IIT (4 projects: Translation of History, Arroyo Campus, Mosaic Fieldhouse, Nordic Assembly)
- Pinterest board at `research/pinterest-board/` (~95 images, architecture school references + graphics)

**You must view reference images before any design or wireframe work.** Written descriptions of aesthetics are not sufficient — they produce generic output.
...
- **One task = one branch = one PR.** Never bundle multiple tasks.
...
- **Wireframes/visual concepts always need human review** before implementation begins
- **Stay in scope** — if a task says "create design system components," don't also refactor every page file
...
- **Scope discipline:** V1 = Asimov-level quality (typography, restraint, editorial pacing). Don't let "play within polish" become scope creep — nail the polish first.
- **Adobe Fonts ≠ self-host:** Adobe Fonts (Creative Cloud) does not permit exporting .woff2 for self-hosting. Options: (1) use Adobe's hosted web project CSS embed, (2) buy separate web license from MyFonts/foundry for `next/font/local`, or (3) keep Google Fonts stand-ins (Jost, DM Mono). Documented in `brand-guide.md` §4.
- **Wireframes/visual concepts = needs_human:** Never close a design/wireframe task without human review. Visual concepts require Julianna's approval before implementation begins.
- **Ship visible output first:** Build the page, create components as needed along the way. Don't build a component library for a page that doesn't exist. Infrastructure-first is the wrong priority.
- **Visual-first design:** Agents must SEE visual references (images) before designing or wireframing — written descriptions of aesthetics produce generic output. View every image in `research/moodboard/` before design work.
- **Wireframes = HTML, not text:** Build wireframes as standalone HTML files or directly in the Next.js app. Text/ASCII wireframes don't give the human enough to react to.

## Lattice (Task Tracking)
...
Write the plan before moving to `planned`:
...
4. Move to `planned` only when the plan file reflects what you intend to build.

**The test:** If you moved to `planned` and the plan file is still empty scaffold, you didn't plan. The CLI enforces this: transitioning to `in_progress` is blocked when the plan is still scaffold.

### The Review Gate

Moving to `review` is a commitment to actually review the work. Review should:
...
5. Record findings with `lattice comment --role review`.

### When You're Stuck

Use `needs_human` when you need human decision, approval, or input.
...
Every operation requires `--actor`. Use `agent:codex` for Codex-initiated work.

### Branch Linking

Link feature branches to tasks: `lattice branch-link <task> <branch-name> --actor agent:<your-id>`.

### Leave Breadcrumbs

Use `lattice comment` for what you tried, chose, and left undone. The record you leave is the only bridge to the next agent's context.

### Shared Worktree Discipline

Multiple agents may work in the same repository concurrently on different tasks.
...
2. **Never revert, reset, or delete changes you can't attribute.** If you're unsure, leave them alone.
